    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(customer.name, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
            const Text('Outstanding balance', style: TextStyle(color: AppColors.muted)),
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Payment received', prefixText: 'Rs  '),
            ),
            const SizedBox(height: 14),
            const Text('You can record partial payments at any time.', style: TextStyle(color: AppColors.muted)),
            const Spacer(),
            FilledButton(
              onPressed: _isSaving
                  ? null
                  : () async {
                      ...
                      try {
                        ...
                        final message = error.response?.data?['message']?.toString() ?? 'Unable to save payment.';
                        if (!context.mounted) return;
