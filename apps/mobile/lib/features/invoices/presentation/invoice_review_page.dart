...
class InvoiceReviewPage extends ConsumerStatefulWidget {
  const InvoiceReviewPage({required this.extractionId, super.key});
...
class _InvoiceReviewPageState extends ConsumerState<InvoiceReviewPage> {
  ...
  Widget build(BuildContext context) {
    final extractionAsync = ref.watch(invoiceExtractionProvider(widget.extractionId));
  ...
  void _maybeAutoSelectContact(List<Customer> contacts) {
    ...
    final match = contacts.where((c) => c.name.trim().toLowerCase() == name).firstOrNull;
  ...
  Widget _buildForm(InvoiceExtraction extraction) {
    ...
    final sectionTitle = Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800);
      ...
      child: ListView(
        ...
        children: [
          Container(
            ...
            child: const Center(
              child: Icon(Icons.receipt_long_rounded, size: 54, color: AppColors.muted),
            ),
          ),
          ...
          Container(
            ...
            child: const Row(
              children: [
                ...
                SizedBox(width: 10),
                Expanded(
                  child: Text('AI extracted these details. Review and correct anything before approving.'),
                ),
              ],
            ),
          ),
          ...
          if (_draftCustomerName != null && _draftCustomerName!.trim().isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(top: 4, bottom: 8),
              child: Text('OCR detected: "$_draftCustomerName"', style: const TextStyle(color: AppColors.muted, fontSize: 12)),
            )
          else
          ...
          contactsAsync.when(
            data: (contacts) => DropdownButtonFormField<String>(
              ...
              items: [
                for (final contact in contacts) DropdownMenuItem(value: contact.id, child: Text(contact.name)),
              ],
              onChanged: (value) => setState(() => _selectedContactId = value),
              validator: (value) => value == null || value.isEmpty ? 'Select the customer for this invoice.' : null,
            ),
            ...
            error: (error, stackTrace) => Row(
              children: [
                const Expanded(child: Text('Unable to load customers.', style: TextStyle(color: AppColors.danger))),
                TextButton(onPressed: () => ref.invalidate(receivablesProvider), child: const Text('Retry')),
              ],
            ),
          ),
          ...
          _buildDateField(label: 'Invoice date', value: _invoiceDate, onTap: _pickInvoiceDate),
          const SizedBox(height: 12),
          _buildDateField(
          ...
          for (var i = 0; i < _items.length; i++)
            Padding(padding: const EdgeInsets.only(bottom: 12), child: _buildItemCard(i)),
          const SizedBox(height: 8),
          TextFormField(
            controller: _totalController,
            decoration: const InputDecoration(labelText: 'Invoice total', prefixText: 'Rs  '),
            keyboardType: const TextInputType.numberWithOptions(decimal: true),
            validator: (value) {
              final parsed = double.tryParse((value ?? '').trim());
              if (parsed == null || parsed <= 0) return 'Enter a valid invoice total.';
              return null;
            },
          ),
          if (_validationErrors.isNotEmpty)
            Padding(
              ...
              child: Container(
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(color: const Color(0xFFFDEAE8), borderRadius: BorderRadius.circular(12)),
                child: Column(
    ...
    return InkWell(
      ...
      child: InputDecorator(
        ...
        child: Text(
          ...
          style: TextStyle(color: value != null ? AppColors.ink : AppColors.muted),
        ),
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            const Text('Invoice extraction failed', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 18)),
            const SizedBox(height: 8),
            Text(
              extraction.failureReason ?? 'The OCR service could not read this invoice.',
              style: const TextStyle(color: AppColors.muted),
              textAlign: TextAlign.center,
            ),
            const SizedBox(height: 20),
            FilledButton(onPressed: () => context.go('/upload'), child: const Text('Try uploading again')),
          ],
        ),
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Still reading your invoice…', style: TextStyle(color: AppColors.muted)),
            const SizedBox(height: 16),
            TextButton(
              onPressed: () => ref.invalidate(invoiceExtractionProvider(widget.extractionId)),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.check_circle_rounded, size: 48, color: AppColors.brand),
            const SizedBox(height: 12),
            const Text(
            ...
            const SizedBox(height: 20),
            FilledButton(onPressed: () => context.go('/receivables'), child: const Text('Go to receivables')),
          ],
        ),
      ),
    );
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                OutlinedButton(onPressed: () => context.go('/upload'), child: const Text('Back to upload')),
  ...
  Future<void> _approve() async {
    ...
    for (final item in _items) {
      ...
      if (!isValid) {
        errors.add('Fix the invalid values for "${name.isEmpty ? 'unnamed item' : name}".');
        continue;
      }

      approvalItems.add(InvoiceApprovalItem(name: name, quantity: quantity, unitPrice: unitPrice, lineTotal: lineTotal));
      itemsSum += lineTotal;
    }

    if (total != null && approvalItems.isNotEmpty && (total - itemsSum).abs() > 1) {
      errors.add(
    ...
    try {
      await ref.read(invoiceApiProvider).approveInvoice(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(behavior: SnackBarBehavior.floating, content: Text('Invoice approved and added to receivables.')),
      );
      ...
      if (mounted) {
        setState(() => _validationErrors = const ['Unable to approve the invoice. Please try again.']);
      }
    } finally {
  ...
  void _handleApprovalError(DioException error) {
    ...
    if (data is Map<String, dynamic> && data['message'] != null) {
      ...
      message = 'This invoice extraction or customer could not be found. It may have expired.';
    } else if (status == 409) {
...
class _ItemEditor {
  _ItemEditor({
    ...
    this.ocrText,
       ...
       quantityController = TextEditingController(text: _formatOrEmpty(quantity)),
       unitPriceController = TextEditingController(text: _formatOrEmpty(unitPrice)),
       lineTotalController = TextEditingController(text: _formatOrEmpty(lineTotal));
...
class _ConfidenceBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final label = confidence.isEmpty ? confidence : '${confidence[0].toUpperCase()}${confidence.substring(1)}';
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
      decoration: BoxDecoration(color: color.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(20)),
      child: Text('$label confidence', style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.w700)),
    );
  }
}
...
class _CrossedOutBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Text('Crossed out on invoice', style: TextStyle(color: AppColors.danger, fontSize: 11, fontWeight: FontWeight.w700)),
    );
  }
}
